...
function BroadcastModal({ selectedEmails, onClose }) {
  ...
  const inp = { width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '10px 14px', color: '#fff', fontFamily: 'Montserrat, sans-serif', fontSize: 13, outline: 'none', boxSizing: 'border-box' }
  const inpFocusStyle = `
    .pt-input::placeholder { color: rgba(255,255,255,0.2); font-style: italic; }
    .pt-input:focus { border-color: rgba(200,169,110,0.4) !important; background: rgba(255,255,255,0.08) !important; }
  `
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{
      ...
      <style>{`
        .pt-modal-input::placeholder { color: rgba(255,255,255,0.2); font-style: italic; }
        .pt-modal-input:focus { border-color: rgba(200,169,110,0.5) !important; background: rgba(255,255,255,0.09) !important; outline: none; }
      `}</style>
      <div style={{ width: '100%', maxWidth: 540, background: '#111', border: '1px solid rgba(200,169,110,0.2)', borderRadius: 20, padding: 32, fontFamily: 'Montserrat, sans-serif' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(255,255,255,0.2)', marginBottom: 5 }}>From <span style={{ color: 'rgba(255,255,255,0.12)', fontWeight: 400, letterSpacing: 0 }}>(fixed)</span></div>
          <div style={{ ...inp, color: 'rgba(255,255,255,0.3)', cursor: 'default', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', fontStyle: 'italic' }}>info@precision-training.io · Precision Training</div>
        </div>

        {/* Subject */}
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(200,169,110,0.7)', marginBottom: 5 }}>Subject ✏️</div>
          <input
            className="pt-modal-input"
            value={subject}
            onChange={e => setSubject(e.target.value)}
            placeholder="z.B. Dein Plan wartet auf dich 🎯"
            style={{ ...inp, border: '1px solid rgba(200,169,110,0.25)' }}
          />
        </div>

        {/* Message */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(200,169,110,0.7)', marginBottom: 5 }}>Message ✏️</div>
          <textarea
            className="pt-modal-input"
            value={message}
            ...
            style={{ ...inp, resize: 'vertical', lineHeight: 1.7, border: '1px solid rgba(200,169,110,0.25)' }}
          />
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', marginTop: 5 }}>✍️ Klick ins Feld und schreib deinen Text — plain text, persönlich.</div>
        </div>
...
export default function Admin() {
  ...
  const base = {
    ...
    btnMuted: { padding: '9px 16px', borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.55)', fontFamily: 'Montserrat, sans-serif', fontSize: 10, fontWeight: 700, letterSpacing: 1, cursor: 'pointer' },
    ...
    <div style={base.page}>
      ...
      <div style={base.body}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button style={base.btnMuted} onClick={loadPlans}>↺ Refresh</button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, minHeight: 28 }}>
          ...
          {selected.size > 0 && (
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setSelected(new Set())} style={{ ...base.btnMuted, padding: '7px 12px' }}>Clear</button>
