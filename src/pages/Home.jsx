...
export default function Home() {
  ...
  return (
    <div className={styles.page}>
      ...
      {/* ── HERO ──────────────────────────────────────────── */}
      <section className={styles.heroSection} style={{ padding: '80px 24px 60px', textAlign: 'center', maxWidth: 760, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 'clamp(38px, 7vw, 64px)', fontWeight: 900, lineHeight: 1.05, marginBottom: 22, color: '#fff', letterSpacing: '-0.02em' }}>
          Your Body. Your Plan.<br /><span style={{ color: '#c8a96e' }}>Built by AI.</span>
        </h1>
        <p style={{ fontSize: 17, color: 'rgba(255,255,255,0.55)', lineHeight: 1.65, marginBottom: 36, maxWidth: 580, margin: '0 auto 36px' }}>
          Get a fully personalized training and nutrition plan calculated from your exact body data, goals, and schedule. Progressive overload built in. Delivered in minutes.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 20 }}>
          <Link to="/form/training" style={{ display: 'inline-block', padding: '16px 32px', borderRadius: 50, background: 'linear-gradient(135deg,#a88a50,#c8a96e)', color: '#000', fontSize: 14, fontWeight: 800, letterSpacing: 0.5, textDecoration: 'none', boxShadow: '0 8px 28px rgba(200,169,110,0.35)', transition: 'transform 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'}
            onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
            Training Plan →
          </Link>
          <Link to="/form/nutrition" style={{ display: 'inline-block', padding: '16px 32px', borderRadius: 50, background: 'transparent', border: '1.5px solid rgba(200,169,110,0.4)', color: '#c8a96e', fontSize: 14, fontWeight: 800, letterSpacing: 0.5, textDecoration: 'none', transition: 'all 0.2s' }}
        ...
        <div style={{ padding: '12px 20px', borderRadius: 10, background: 'rgba(109,184,138,0.06)', border: '1px solid rgba(109,184,138,0.2)', display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
          <span style={{ fontSize: 16 }}>💊</span>
          <span style={{ fontSize: 12, color: '#6db88a', fontWeight: 600 }}>On GLP-1 medication?</span>
          <Link to="/form/glp1" style={{ fontSize: 12, color: '#6db88a', fontWeight: 700, textDecoration: 'underline', textUnderlineOffset: 2 }}>Get Muscle Guard Plan →</Link>
        </div>
        <div style={{ marginTop: 16, fontSize: 11, color: 'rgba(255,255,255,0.25)', letterSpacing: 0.5 }}>100% free · No subscription · Delivered in 1-5 minutes</div>
      </section>
