  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.heroSection} style={{ padding: '80px 24px 60px', textAlign: 'center', maxWidth: 720, margin: '0 auto' }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2.5, textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 16 }}>AI-Powered Fitness & Nutrition</div>
        <h1 style={{ fontSize: 'clamp(32px, 6vw, 56px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 20, color: '#fff' }}>
          Your body. Your plan.<br /><span style={{ color: 'var(--gold)' }}>Built in minutes.</span>
        </h1>
        <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 32, maxWidth: 540, margin: '0 auto 32px' }}>
          Get a fully personalized training or nutrition plan generated from your exact body data—free, no subscription, ready in 1–5 minutes.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 24 }}>
          ...
          <Link to="/form/nutrition" style={{ display: 'inline-block', padding: '16px 32px', borderRadius: 50, background: 'transparent', border: '1.5px solid rgba(200,169,110,0.5)', color: 'var(--gold)', fontSize: 14, fontWeight: 800, letterSpacing: 1, textDecoration: 'none', transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(200,169,110,0.08)'; e.currentTarget.style.borderColor = 'var(--gold)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(200,169,110,0.5)'; e.currentTarget.style.transform = 'translateY(0)' }}>
            Get Nutrition Plan →
          </Link>
        </div>
        <div style={{ padding: '12px 20px', borderRadius: 12, background: 'rgba(109,184,138,0.06)', border: '1px solid rgba(109,184,138,0.2)', display: 'inline-block', marginBottom: 16 }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#6db88a', marginBottom: 4 }}>💊 On GLP-1 medication?</div>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6, marginBottom: 10 }}>We have specialized plans that preserve muscle during rapid weight loss.</div>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/form/glp1" style={{ padding: '8px 16px', borderRadius: 20, background: 'linear-gradient(135deg,#4a9e68,#6db88a)', color: '#fff', fontSize: 11, fontWeight: 700, letterSpacing: 0.5, textDecoration: 'none', transition: 'transform 0.15s', whiteSpace: 'nowrap' }}
              onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-1px)'}
              onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
              GLP-1 Training →
            </Link>
            <Link to="/form/glp1-nutrition" style={{ padding: '8px 16px', borderRadius: 20, border: '1px solid rgba(109,184,138,0.3)', background: 'rgba(109,184,138,0.08)', color: '#6db88a', fontSize: 11, fontWeight: 700, letterSpacing: 0.5, textDecoration: 'none', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(109,184,138,0.15)'; e.currentTarget.style.transform = 'translateY(-1px)' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(109,184,138,0.08)'; e.currentTarget.style.transform = 'translateY(0)' }}>
              GLP-1 Nutrition →
            </Link>
          </div>
        </div>
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', letterSpacing: 0.5 }}>No subscription · No credit card · Ready in minutes</div>
      </section>
